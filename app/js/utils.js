...
export async function getCategory3(allCategories) {
  const allMovies = await FetchDatas();
  const categoriesWithMovies = allCategories.map(category => {
    const moviesInCategory = allMovies.filter(movie => movie.genres.includes(category.name))
    return {
      category,
      moviesInCategory: moviesInCategory.sort((a, b) => parseFloat(b.imdb_score) - parseFloat(a.imdb_score)).slice(0, 6)
    };
